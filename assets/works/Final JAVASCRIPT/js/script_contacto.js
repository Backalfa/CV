function validar_contacto(){
    let thumbs_up = document.querySelector("#thumbs_up");

    if( todos_campos() == true && validar_nome() == true && validar_apelido() == true && validar_telemovel() == true && validar_email() == true && validar_data() == true){
    console.log(todos_campos());
    ...
    thumbs_up.classList.remove("hidden");
    alert("Formulário enviado com Sucesso!");

    }
}

function todos_campos(){
    let telemovelValue = document.querySelector("#telemovel").value;
    let dataValue = document.getElementById("datatime").value;
    let nome = document.querySelector("#nome").value;
    let email = document.querySelector("#email").value;
    let motivo = parseInt(document.querySelector("#motivo").value);
    if( motivo === 1 || nome.length == 0 || email.length == 0 || telemovelValue.length == 0 || dataValue.length == 0){
